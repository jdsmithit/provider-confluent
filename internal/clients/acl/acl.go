...
func (c *Client) ACLCreate(acl v1alpha1.ACLBlock) (ACLResponseBlock, error) {
	var resp ACLResponseBlock

	cmd, err := commands.NewACLCreateCommand(acl)
	if err != nil {
		return resp, err
	}
...
func (c *Client) ACLDelete(acl v1alpha1.ACLBlock) error {
	cmd, err := commands.NewACLDeleteCommand(acl)
	if err != nil {
		return err
	}
...
func (c *Client) ACLList(serviceAccount string, environment string, cluster string) (ACLResponseBlockList, error) {
	var resp ACLResponseBlockList

	cmd := commands.NewACLListCommand(serviceAccount, environment, cluster)
	out, err := clients.ExecuteCommand(exec.Cmd(cmd))
